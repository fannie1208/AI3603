...
def parse_args(): 
    ...
    parser.add_argument("--gamma", type=float, default=1, help="the discount factor gamma")
    ...
    parser.add_argument('--result_path', type=str, default='results/sarsa/', help='Path where the result will be saved in.')
...
experiment_name = f'sarsa.alpha_{args.alpha}.gamma_{args.gamma}.epsilon_{args.e}.de_{args.de}.da_{args.da}.seed_{args.seed}.{datetime_now}'
...
agent = SarsaAgent(all_actions, epsilon=args.e, alpha=args.alpha, gamma=args.gamma) 
...
for episode in range(1000):
    ...
    logger.info(f"=====SarsaTable: {agent.Q_list}")
